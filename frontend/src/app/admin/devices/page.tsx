  ...
  async function loadDevices() {
    ...
    try {
      ...
      if (error) {
        ...
        // 1. Explicitly type the raw data as 'any' to stop the deep nesting check
        const rawData = data as any[];

        // 2. Map and normalize
        const normalized: DeviceRow[] = (rawData ?? []).map((row) => ({
          id: row.id,
          cpu_id: row.cpu_id,
          ...
          // Extract the first item from the array returned by Supabase joins
          showroom: Array.isArray(row.showroom) ? row.showroom[0] : row.showroom || null,
          requester: Array.isArray(row.requester) ? row.requester[0] : row.requester || null,
        }));

        // 3. Update state with the clean, flat objects
                      ...
                      <td className="p-6">
                        {d.awaiting_approval ? (
                          <div className="flex flex-wrap gap-2">
                            <button
                              onClick={async () => {
                                const result = await approveDevice(d.id);
                               if (!result?.success) {
                                 alert(result?.message || "Failed to approve device");
                                 return;
                               }
                               await loadDevices();
                             }}
                            ...
                            <button
                              onClick={async () => {
                                ...
                                const result = await replaceDevice(d.id, d.showroom?.id ?? "");
                                if (!result?.success) {
                                  alert(result?.message || "Failed to replace device");
                                  return;
                                }
                                await loadDevices();
                              }}
                            ...
                            <button
                             onClick={async () => {
                               ...
                               const result = await rejectDevice(d.id);
                               if (!result?.success) {
                                 alert(result?.message || "Failed to reject device");
                                 return;
                               }
                               await loadDevices();
                             }}
