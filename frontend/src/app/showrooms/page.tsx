...
export default function ShowroomsPage() {
  ...
  const { session, loadingSession, setActiveShowroomId } = useSession();
  ...
  useEffect(() => {
    ...
    const loadShowrooms = async () => {
      ...
      if (session.role === "manager" && session.showroom) {
            ...
            <div style={statCardStyle}>
              ...
              <div>
                <h3 style={statValueStyle}>
                  {formatCurrency(stats.totalBalance, showrooms[0]?.currency_code ?? "TND")}
                </h3>
